...
public class TransaccionProveedorLogic {
    ...
    public TransaccionProveedorEntity createTransaccionProveedor( Long proveedorId,TransaccionProveedorEntity transaccionProveedorEntity){
        ...
        return persistence.create(transaccionProveedorEntity);
    ...
    public TransaccionProveedorEntity getTransaccionProveedor(Long proveedorId,Long transaccionProveedorId) {
        LOGGER.log(Level.INFO, String.format("Inicia proceso de consultar la transaccion con id = {0} del proveedor con id = %d", proveedorId), transaccionProveedorId);
    ...
    public TransaccionProveedorEntity updateTransaccionProveedor(Long proveedorId, TransaccionProveedorEntity transaccionProveedorEntity) {
        LOGGER.log(Level.INFO, String.format("Inicia proceso de actualizar la transaccionProveedor con id = {0} del proveedor con id = ", proveedorId), transaccionProveedorEntity.getId());
        ProveedorEntity proveedorEntity = proveedorPersistence.find(proveedorId);
        ...
        LOGGER.log(Level.INFO, String.format("Termina proceso de actualizar la transaccionProveedor con id = {0} del proveedor con id", proveedorId), transaccionProveedorEntity.getId());
        return newEntity;
    }
    ...
    public void deleteTransaccionProveedor(Long proveedorId, Long transaccionProveedorId) throws BusinessLogicException {
        LOGGER.log(Level.INFO, String.format("Inicia proceso de borrar la transaccionProveedor con id = {0} del proveedor con id %d", proveedorId), transaccionProveedorId);
        TransaccionProveedorEntity old = getTransaccionProveedor(proveedorId, transaccionProveedorId);
        if (old == null) {
            throw new BusinessLogicException(String.format("la transaccion con id = %d no esta asociado a el cliente con id = %d",transaccionProveedorId, proveedorId));
        }
